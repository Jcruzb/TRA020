...
export function validateHistory(history) {
  if(history?.schemaVersion!==1 || !Array.isArray(history.jobs)) throw new Error('El historial no tiene un formato compatible. No se ha sobrescrito.');
  const ids=new Set();
  for(const job of history.jobs){
    if(!segment.test(job.id) || ids.has(job.id) || !Array.isArray(job.selectedIds) || !job.annex || !job.signed || !Array.isArray(job.exports)) throw new Error('El historial contiene trabajos inválidos. No se ha sobrescrito.');
...
async function readText(directory,name){
  try{return await(await directory.getFileHandle(name)).getFile().then(f=>f.text());}
...
export class LocalHistory {
  constructor(directory,history,text){this.directory=directory;this.history=history;this.lastText=text;this.queue=Promise.resolve();}
  static async open(directory){
    const text=await readText(directory,'historial.json');
    let history;
    try{history=text===null?emptyHistory():validateHistory(JSON.parse(text));}
    catch(error){throw new Error(`No se puede abrir historial.json: ${error.message} Revisa el archivo o recupera historial.anterior.json.`);}
  ...
  async persist(history){
    ...
    if(current!==this.lastText)throw new Error('El historial cambió en otra ventana. Vuelve a abrir la carpeta antes de guardar para evitar sobrescribir trabajos.');
    const next={...history,updatedAt:new Date().toISOString()};
    const text=JSON.stringify(next,null,2);
    if(current!==null)await writeFile(this.directory,'historial.anterior.json',current);
    await writeFile(this.directory,'historial.json',text);
    this.lastText=text;this.history=next;
    return next;
  }
  enqueue(action){const next=this.queue.then(action);this.queue=next.catch(()=>{});return next;}
  saveJob(job){
    const snapshot=structuredClone(job);
    return this.enqueue(async()=>{
      validateHistory({schemaVersion:1,jobs:[snapshot]});
      const jobs=[snapshot,...this.history.jobs.filter(j=>j.id!==snapshot.id)].sort((a,b)=>b.updatedAt.localeCompare(a.updatedAt));
  ...
  async saveAttachment(jobId,id,file){
    ...
    const root=await this.directory.getDirectoryHandle('TRA020_archivos',{create:true});
    ...
    return {name:file.name,path:`TRA020_archivos/${jobId}/${name}`,size:file.size,type:file.type,attachedAt:new Date().toISOString()};
  }
  async readAttachment(meta){
    const parts=meta.path.split('/');
    if(parts.length!==3 || parts[0]!=='TRA020_archivos' || !segment.test(parts[1]) || !/^[a-zA-Z0-9_.-]+$/.test(parts[2]))throw new Error('Ruta de adjunto inválida.');
    try{
      ...
      if(file.size!==meta.size)throw new Error('El tamaño del adjunto cambió. Vuelve a adjuntar el archivo.');
      return {name:meta.name,bytes:await file.arrayBuffer()};
    }catch(error){throw new Error(`No se puede recuperar ${meta.name}: ${error.message}`);}
  }
}
